...
void fix_image_cpu(Image &to_fix) {
    ...
    for (int i = 0; i < to_fix.size(); ++i)
        if (to_fix.buffer[i] != garbage_val) predicate[i] = 1;
    ...
    for (std::size_t i = 0; i < predicate.size(); ++i)
        if (to_fix.buffer[i] != garbage_val) to_fix.buffer[predicate[i]] = to_fix.buffer[i];
    ...
    histo.fill(0);
    for (int i = 0; i < image_size; ++i) ++histo[to_fix.buffer[i]];
    ...
    std::transform(to_fix.buffer, to_fix.buffer + image_size, to_fix.buffer, [image_size, cdf_min, &histo](int pixel) {
